...
for iter = 1: (nsave + nburn)
    ...
    for l = 1:ncomp
        ...
        if (~isempty(points))
            ...
            Dstarg = truncnormD(mu_dg,V_dg,Dg,'trandn');
            
            % ==========|| Draw the covariance params
            e_vg   = Dstarg - z_use*gammag;
            V_kg   = (e_vg'*e_vg/sigesqg + inv(tau0*sigesqg))\1;
            mu_kg = V_kg*( inv(tau0*sigesqg)*mu_k0 + e_vg'*(Yg - x_use*betag)/sigesqg );
            siguvg = mu_kg + chol(V_kg)'*randn(1,1);
            
            sse_Yg  = (Yg-x_use*betag-siguvg*e_vg)'*(Yg-x_use*betag-siguvg*e_vg);
            sigesqg = 1./gamrnd( c_e0+siguvg^2/(2*tau0)+0.5*npts, 1./(d_e0+0.5*sse_Yg) ); 
            
            Sigg = [sigesqg+siguvg^2 siguvg;
                        siguvg 1];
            ...
            thetag = meanpart + (chol(covmatpart))'*randn((pbar),1);
            
            gammag = thetag(1:pz,1);
            betag  = thetag(pz+1:pbar,1);
        ...
        clear points Yg Dg Dstarg z_use x_use betag gammag siguvg sigesqg Siginvg
    end
    
%     [TE, ind] = sort(beta(2,:), 'descend');
%     beta             = beta(:,ind);
%     gamma            = gamma(:,ind);
%     omega            = omega(:,ind);
%     siguv            = siguv(:,ind);
